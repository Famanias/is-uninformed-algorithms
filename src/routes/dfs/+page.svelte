...
<script lang="ts">
    ...
    function runDFS() {
      if (graph) {
        ...
        const result = dfs(graphFormat, startNode.toString(), goalNode.toString());
        if (result) {
          traversalResult = result.map(Number);
        } else {
          traversalResult = [];
          const visited = new Set<string>();
          const stack: string[] = [startNode.toString()];
          while (stack.length > 0) {
            const node = stack.pop()!;
            if (!visited.has(node)) {
              visited.add(node);
              traversalResult.push(Number(node));
              const neighbors = graphFormat[node] || [];
  ...
  <main>
    ...
    <div class="controls">
      ...
      <button on:click={runDFS}>Run DFS</button>
    </div>
    ...
    <div class="results">
      ...
      {:else}
        <p>Click "Run DFS" to see the traversal</p>
      {/if}
    </div>
  </main>