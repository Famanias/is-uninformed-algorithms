...
<script lang="ts">
  ...
  function runIDDFS() {
    if (graph) {
      ...
      const result = iddfs(graphFormat, startNode.toString(), goalNode.toString(), maxDepth);
      if (result) {
        traversalResult = result.map(Number);
      } else {
        traversalResult = [];
        const visited = new Set<string>();
        function dls(node: string, depth: number, depthLimit: number) {
          ...
          if (!visited.has(node)) {
            ...
            for (const neighbor of graphFormat[node] || []) {
              if (!visited.has(neighbor)) {
                dls(neighbor, depth + 1, depthLimit);
              }
            }
          }
        }
        for (let depth = 0; depth <= maxDepth; depth++) {
          visited.clear();
          traversalResult = [];
          dls(startNode.toString(), 0, depth);
        }
      }
...
<main>
  <h1>Iterative Deepening Depth-First Search Demo</h1>
  ...
  <div class="controls">
    ...
    <label>
      Max Depth:
      <input type="number" min="0" max="5" bind:value={maxDepth}>
    </label>
    <button on:click={runIDDFS}>Run IDDFS</button>
  </div>
  ...
  <div class="results">
    ...
    {:else}
      <p>Click "Run IDDFS" to see the traversal</p>
    {/if}
  </div>
</main>