...
<script lang="ts">
    ...
    function runBFS() {
      if (graph) {
        ...
        const result = bfs(graphFormat, startNode.toString(), goalNode.toString());
        if (result) {
  ...
  <main>
    <h1>Breadth-First Search Demo</h1>
    ...
    <div class="controls">
      ...
      <button on:click={runBFS}>Run BFS</button>
    </div>
    ...
    <div class="results">
      ...
      {:else}
        <p>Click "Run BFS" to see the traversal</p>
      {/if}
    </div>
  </main>